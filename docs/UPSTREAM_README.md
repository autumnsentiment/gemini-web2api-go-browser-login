...
<img src="banner.svg" alt="gemini-web2api-go" width="100%">
...
- `/v1/images/generations`、`/v1/images/edits`：OpenAI 图像 API 形状。内部转成 `gemini-image`
  那条链（`POST` 文生图 / 图生图），结果以 `data[].b64_json` 回传（要登录态；`edits` 是 multipart）
...
| `POST /v1/images/generations` | ✅ | OpenAI 图像 API 形状。JSON `{model:"gemini-image", prompt, n, size, response_format, image?}`；带 `image`（data URL / http 链接）即为图生图。结果 `data[].b64_json`（`response_format:"url"` 时另给 `data` URL）。要登录态 cookie |
| `POST /v1/images/edits` | ✅ | multipart/form-data：`image`（可重复）/ `mask` / `prompt` / `model` / `n` / `size` / `response_format`。源图也接受字符串部件（data URL）。同样走 `gemini-image`，结果 `data[].b64_json` |
| `/v1/embeddings`、`/v1/audio/*` | ❌ | 未实现，返回 404 |
