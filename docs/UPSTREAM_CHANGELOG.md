...
- **`/v1/images/generations` + `/v1/images/edits` 端点**。OpenAI 图像 API 形状，内部转成
  `gemini-image` 那条链，产物以 `data[].b64_json` 回传（`response_format:"url"` 时另给 data URL）。
  `edits` 走 multipart（`image` 可重复 + `mask` + `prompt`），也兼容 JSON + data URL。
  以前只有 chat 端点能生图，很多客户端（newapi、Cherry Studio、LobeChat 的画图页）不认，
  选模型时根本选不到 `gemini-image`。要登录态 cookie。

### 修复

- **图生图偶发「回声」**。带附件的 `gemini-image` 请求实测约 5 次里 1 次会被上游丢掉
  `inner[49]=14`、退回纯文本模型（响应自带模型报 `3.6 Flash`），它不生图、只把用户附件
  原样引用回来 —— 客户端会拿到一张跟自己上传的图一模一样的「修改后」图片。现在按**解码后的
  像素**比对识别回声（上游会重新编码，字节比对抓不住），丢掉重打并记 `[images]` 日志；
  带附件的请求重试预算放宽到 8 次。实测 3 次里 1 次的失败率降到 0。
