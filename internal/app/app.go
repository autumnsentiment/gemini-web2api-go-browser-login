...
func Run() {
	...
	if *browserCDPHost != "" {
		cfg.BrowserCDPHost = *browserCDPHost
	}
	if *browserAccessURL != "" {
		cfg.BrowserAccessURL = *browserAccessURL
	}
	if envB := os.Getenv("BROWSER_CONTROLLER_URL"); envB != "" && cfg.BrowserControllerURL == "" {
		cfg.BrowserControllerURL = envB
	}
	if envH := os.Getenv("BROWSER_CDP_HOST"); envH != "" && cfg.BrowserCDPHost == "" {
		cfg.BrowserCDPHost = envH
	}
	if envA := os.Getenv("BROWSER_ACCESS_URL"); envA != "" && cfg.BrowserAccessURL == "" {
		cfg.BrowserAccessURL = envA
	}
