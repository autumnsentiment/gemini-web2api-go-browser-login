...
### 扩展配置流程（本机浏览器方案，实测于 2026-09-17）

面板「初始化引导」页按部署环境给出同样的步骤，每步附「查看本步参数」弹窗。
以下为本机 Chrome 实测记录：

**① 安装扩展**：下载扩展 zip 并解压 → `chrome://extensions` → 开启开发者模式 →
「加载未打包的扩展程序」选解压目录 → 卡片出现「Gemini Cookie Sync 1.1.0」。

**② 配置扩展**（点工具栏扩展图标打开弹窗）：

| 参数 | 填写值 | 说明 |
|---|---|---|
| Profile 标识 | 自定义（如 `本机浏览器`） | 入池后账号显示为 `remote:<标识>` |
| **推送模式** | **服务端（远程服务器，推荐）** | ★ 默认是「控制器」，必须手动切换——最容易踩的坑 |
| 目标地址 | `http://<服务器IP>:8083` | 本服务监听端口；不是控制器的 9280 |
| 密钥 | 面板「设置」页的 API Key | 服务端模式用它鉴权 |
| 启用扩展 / 自动保活 | 勾选 | 保活 10 分钟、入池 30 分钟为默认 |

填完**必须点「保存」**。

**③ 登录并入池**：本机浏览器登录 gemini.google.com → 扩展弹窗点「立即入池」→
弹窗显示「✓ cookie 已入库并完成模型校验」、Cookie 池出现 `remote:` 前缀账号即成功。
之后无需再管：扩展每 30 分钟自动入池、每 10 分钟自动保活。

**常见问题**：`401` = API Key 不对或没保存；`Failed to fetch` = 地址/端口不可达；
`400 bad json` = 扩展版本过旧（重新下载）；「校验未通过」= 多为出口被拦，稍后重试。
